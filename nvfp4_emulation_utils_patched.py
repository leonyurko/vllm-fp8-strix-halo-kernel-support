...
def run_nvfp4_emulations(
    ...
    import os as _os
    if x_m == 1 and (not swizzle) and _os.environ.get("NVFP4_GEMV","1") == "1":
        try:
            import torch as _t
            from nvfp4_triton import nvfp4_gemv
            if _os.environ.get("NVFP4_SKIP_ACT", "0") == "1":
                _xdq = x.to(_t.float32)
            else:
                _xf, _xb = ref_nvfp4_quant(x, input_global_scale, group_size)
                _xf = _xf.reshape(x_m, x_k // group_size, group_size)
                _xb = _xb.unsqueeze(-1) / input_global_scale
                _xdq = (_xf * _xb).reshape(x_m, x_k).to(_t.float32)
            _w = weight.data.view(_t.uint8)
            _ws = weight_scale_swizzled.data.view(_t.float8_e4m3fn).to(_t.float32) * weight_global_scale
            return nvfp4_gemv(_xdq, _w, _ws).to(output_dtype)
        except Exception:
            pass
