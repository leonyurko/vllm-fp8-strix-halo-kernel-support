        ...
        try:
            ...
            if total < 70 * 1024**3: 
                ...
                if drm_cards:
                    ...
                    def _rd(n):
                        try:
                            with open(os.path.join(card_dir, n), 'r') as f:
                                return int(f.read().strip())
                        except Exception:
                            return 0
                    gtt_total = _rd('mem_info_gtt_total')
                    gtt_used  = _rd('mem_info_gtt_used')
                    vram_total = _rd('mem_info_vram_total')
                    vram_used  = _rd('mem_info_vram_used')
                    # BIOS may place the big UMA pool in VRAM (dedicated carve-out)
                    # OR in GTT (small carve-out + large GTT). Use whichever is larger.
                    if vram_total >= gtt_total:
                        real_ceiling, real_used = vram_total, vram_used
                    else:
                        real_ceiling, real_used = gtt_total, gtt_used
                    # carve 8GB off the top for safety
                    safe_ceiling = real_ceiling - (8 * 1024**3)
                    real_total = safe_ceiling
                    real_free = max(0, safe_ceiling - real_used)
...
class RocmPlatform(Platform):
    ...
    def apply_config_platform_defaults(cls, vllm_config: "VllmConfig") -> None:
        ...
        if (
            ...
            if not getattr(self, "on_gfx1x", lambda: False)():
