...
class TorchFP8ScaledMMLinearKernel(FP8ScaledMMLinearKernel):
    ...
    def get_output_padding(self) -> int | None:
        ...
        # gfx1151 fused-Triton path: we route through fp8_gemm (no
        # torch._scaled_mm), and the dedicated M==1 GEMV is the fast decode
        # path. Padding M up to 17 would force decode through the slow tl.dot
        # GEMM, so disable output padding entirely here.
        return None
...
def _scaled_mm_fb(A, B, *, out_dtype, scale_a, scale_b, bias=None):
    # gfx1151: route through the fused FP8->bf16 Triton dequant GEMM (weights
    # stay FP8 in HBM; no full bf16 materialization). fp8_triton is on PYTHONPATH.
    try:
        from fp8_triton import fp8_gemm
        return fp8_gemm(A.contiguous(), B, scale_a, scale_b, out_dtype, bias)  # do NOT .contiguous() B: vLLM passes a transposed weight view; the 200ms/call copy was the bottleneck. Kernel uses B.stride().
    except Exception:
        import torch as _t
        o = (A.to(_t.bfloat16) @ B.to(_t.bfloat16)).to(_t.float32)
        sa = scale_a.to(_t.float32).reshape(-1); sb = scale_b.to(_t.float32).reshape(-1)
        o = o * (sa if sa.numel() == 1 else sa.view(-1, 1))
        o = o * (sb if sb.numel() == 1 else sb.view(1, -1))
        if bias is not None: o = o + bias.to(_t.float32)
